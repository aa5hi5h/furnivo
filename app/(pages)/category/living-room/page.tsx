  ...
  const handleFilterChange = (filters: FilterState) => {
    ...
    if (filters.categories.length > 0) {
      filtered = filtered.filter(p =>
        filters.categories.some((cat:any) => p.name?.toLowerCase().includes(cat.toLowerCase()))
      );
    }

    if (filters.materials.length > 0) {
      filtered = filtered.filter(p =>
        filters.materials.some((mat:any) => p.materials?.includes(mat))
      );
    }

    if (filters.colors.length > 0) {
      filtered = filtered.filter(p =>
        filters.colors.some((col:any) => p.colors?.includes(col))
      );
    }

    if (filters.styles.length > 0) {
      filtered = filtered.filter(p =>
        filters.styles.some((style:any) => p.name?.toLowerCase().includes(style.toLowerCase()))
      );
    }
